...
func (s *HTTPServer) newRouter(_ config.Config) *mux.Router {
	...
	authorized.HandleFunc("/activities/student/{student_id}", s.getActivitiesByStudentID).Methods(http.MethodGet, http.MethodOptions)
	authorized.HandleFunc("/activities/{activity_id}", s.updateActivity).Methods(http.MethodPut, http.MethodOptions)
	authorized.HandleFunc("/activities/{activity_id}", s.deleteActivity).Methods(http.MethodDelete, http.MethodOptions)
